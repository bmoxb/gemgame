...
use super::{tile_coords_to_vec2, SINGLE_TILE_TEXTURE_SIZE};
...
pub struct Renderer {
    ...
    /// Time that has passed since the current movement animation began.
    time_since_movement_began: f32,
    /// Time that has passed since the last animated walk cycle frame change.
    time_since_frame_change: f32,
    /// Store what frame of the animated walk cycle the entity is currently on.
...
struct Movement {
    ...
    movement_time: f32,
    /// The amount of time between each animation frame while this movement is on-going.
...
impl Renderer {
    ...
    pub fn do_movement(
        &mut self, to_coords: TileCoords, movement_time: f32, frame_changes: usize, tile_draw_size: f32
    ) {
        ...
        self.movement_queue.push(Movement {
            ...
            movement_time,
            frame_change_time: movement_time / (frame_changes as f32)
        });
    }
    ...
    pub fn update(&mut self, delta: f32) {
        ...
        self.time_since_frame_change += delta;
        ...
        if let Some(current_movement) = self.movement_queue.first() {
            ...
            if self.time_since_frame_change >= current_movement.frame_change_time {
            ...
            // coordinates exactly, reset the movement timer, and move on to the next movement in the queue:
    ...
    pub fn draw_lower(&self, entity: &Entity, texture: quad::Texture2D, tile_draw_size: f32) {
        self.draw_part(
            texture,
            0.0,
            0.0,
            colours::clothing(entity.clothing_colour),
            body_draw_params(entity, self.walk_frame, tile_draw_size)
        );
    }

    /// Draw the upper portion of the entity (head, face, hands, etc.)
    pub fn draw_upper(&self, entity: &Entity, texture: quad::Texture2D, tile_draw_size: f32) {
        ...
        self.draw_part(
            ...
            tile_draw_size * 0.25,
            skin_colour,
            head_draw_params(entity, self.walk_frame, tile_draw_size)
        );
        ...
        if let Some(x_offset) = right_eye {
            self.draw_part(
                ...
                eye_y_offset,
                hair_colour,
                eye_draw_params(entity, false, tile_draw_size)
            );
        }
        // Left eye:
        if let Some(x_offset) = left_eye {
            self.draw_part(texture, x_offset, eye_y_offset, hair_colour, eye_draw_params(entity, true, tile_draw_size));
        }
        // Mouth:
        if mouth {
            quad::draw_texture_ex(
                ...
                skin_colour,
                mouth_draw_params(entity, tile_draw_size)
            );
        }
        ...
        self.draw_part(
            ...
            hair_colour,
            hair_draw_params(entity, tile_draw_size)
        );
    }
...
fn body_draw_params(entity: &Entity, walk_frame: WalkCycle, tile_draw_size: f32) -> quad::DrawTextureParams {
    let (x_relative, walk_frame_flip) = match walk_frame {
        WalkCycle::BeforeRight | WalkCycle::BeforeLeft => (0, false),
        WalkCycle::Right => (1, false),
        WalkCycle::Left => (1, true)
    };

    let (y_relative, flip) = match entity.direction {
...
fn head_draw_params(entity: &Entity, walk_frame: WalkCycle, tile_draw_size: f32) -> quad::DrawTextureParams {
    let mut params = body_draw_params(entity, walk_frame, tile_draw_size);
    ...
    if let Some(src) = &mut params.source {
        ...
        src.x += (2 * SINGLE_TILE_TEXTURE_SIZE) as f32;
...
fn hair_draw_params(entity: &Entity, tile_draw_size: f32) -> quad::DrawTextureParams {
    ...
    quad::DrawTextureParams {
        ...
        source: Some(quad::Rect {
            x: (x_offset * SINGLE_TILE_TEXTURE_SIZE) as f32,
            y: ((6 + y_offset) * (SINGLE_TILE_TEXTURE_SIZE / 2)) as f32,
            w: SINGLE_TILE_TEXTURE_SIZE as f32,
            h: (SINGLE_TILE_TEXTURE_SIZE / 2) as f32
        }),
        flip_x: flip,
        flip_y: true,
        ..Default::default()
...
fn eye_draw_params(entity: &Entity, left_eye: bool, tile_draw_size: f32) -> quad::DrawTextureParams {
    ...
    quad::DrawTextureParams {
        ...
        source: Some(eye_or_mouth_texture_rect(x_offset, 0)),
        flip_x: left_eye,
        flip_y: true,
        ..Default::default()
    }
...
fn mouth_draw_params(entity: &Entity, tile_draw_size: f32) -> quad::DrawTextureParams {
    ...
    quad::DrawTextureParams {
        dest_size: Some(quad::vec2(tile_draw_size / 2.0, tile_draw_size / 2.0)),
        source: Some(eye_or_mouth_texture_rect(x_relative, 1)),
        flip_y: true,
        ..Default::default()
    }
}

fn eye_or_mouth_texture_rect(x_relative: u16, y_relative: u16) -> quad::Rect {
    quad::Rect {
        x: ((x_relative + 4) * (SINGLE_TILE_TEXTURE_SIZE / 2)) as f32,
        y: ((y_relative + 2) * (SINGLE_TILE_TEXTURE_SIZE / 2)) as f32,
        w: (SINGLE_TILE_TEXTURE_SIZE / 2) as f32,
        h: (SINGLE_TILE_TEXTURE_SIZE / 2) as f32
    }
}
